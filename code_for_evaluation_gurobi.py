...
DT = np.empty((0, 10)) # instead of 10
...
for (J,I) in [(125, 5)]:
    ...
    m = gp.Model("opt_assign_gurobi")
        
    y = m.addMVar(shape = (J,I), vtype=GRB.BINARY, name="y")

    maxobj = m.addMVar(shape=(1),vtype=GRB.INTEGER, name="maxobj")
    comp = m.addMVar(shape=(I),vtype=GRB.INTEGER, name="comp")

    m.addConstr((y.T @ mem_footprint[:,1])  <= mem_capacity_helper)
    m.addConstr( y @ ones_H == ones_K )
       
    m.addConstrs(comp[i] == pij_new[i, :]@y[:,i] for i in range(I))
    max_constr = m.addConstr(maxobj == gp.max_(comp[i] for i in range(I)))
    ...
    if np.sum(mem_footprint[:, 1]) >= np.sum(mem_capacity_helper):
        ...
        try:
            ...
            Y_new = (np.rint(y.x).astype(int)).T
